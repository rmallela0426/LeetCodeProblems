...
vector<int> decode(vector<int>& encoded, int first) {
    // Initialize an array to store the result which of
    // length input encoded length + 1. +1 is for the
    // first element
    vector<int> arr(encoded.size() + 1);

    // Store first element into the array result
    arr[0] = first;

    // Loop all encoded elements
    int i = 0;
    while (i < encoded.size()) {
        // XOR the elem with first elem
        arr[++i] = first = first ^ encoded[i];
    }
